...
config/env-sync.toml  optional per-project list of the key NAMES that project's env file mirrors from Automic Vault; LOCAL, gitignored, firstmate-maintained but human-editable, and NOT inherited by secondmate homes; read only by bin/fm-env-sync.sh, which is the captain's own command because each vault read needs a human approval click no unattended worker can answer; see docs/configuration.md "Project env mirrors"
config/required-checks/<project>  optional per-project list of check names or globs that must be SUCCESS on a pull request's head before bin/fm-pr-merge.sh merges it, standing in for GitHub's required-status-checks rule; LOCAL, gitignored, human-editable, not inherited by secondmate homes, and independent of yolo; see docs/configuration.md "Required checks"
config/secondmate-harness  harness the PRIMARY uses to launch SECONDMATE agents, optionally followed by a model and effort token on the same line ("<harness> [<model>] [<effort>]"; section 4); LOCAL, gitignored; absent or "default" harness falls back to config/crew-harness then firstmate's own. The primary's own setting; NOT inherited into secondmate homes (secondmates do not spawn secondmates)
config/pi-node  absolute path to the node interpreter Pi-family launches run under, so pi does not resolve node through PATH; LOCAL, gitignored, and not inherited; absent means today's PATH behavior; see docs/configuration.md "Pi node interpreter"
...
config/watch-thresholds  optional KEY=value overrides for this home's supervision cadence (pause resurface, stale escalate, signal grace, heartbeat scan); LOCAL, gitignored; inherited by secondmate homes; see docs/configuration.md "Watcher thresholds"
...
state/               runtime records and signals; gitignored
  ...
  <id>.bridge-owner  durable bridge-start ownership record for chrome-devtools-axi cleanup; written atomically by bin/fm-chrome-bridge-sweep.sh at bridge start and consumed by global cleanup
...
When a task belongs on an integration branch rather than the project's default branch, pass that branch to the spawn, the brief, and the eventual PR merge so the worker starts, ships, and lands on it; `docs/configuration.md` owns that contract.
A persistent secondmate is recorded in the secondmate registry and runtime state, never as a backlog work item.

Steer a worker with ordinary text through fail-closed `fm-send`: the message becomes a durable record in the task's steering inbox (multi-line text is legal, local and remote alike) and the worker's terminal receives only a best-effort doorbell line, constant on the initial send and carrying elapsed time on watcher re-rings, with the watcher escalating a stuck one (`bin/fm-task-inbox-lib.sh`; `bin/fm-send.sh` owns the typed-plane carve-outs).
...
If a task must actually run the project, scaffold with `--env-file <absolute path in the primary checkout>[:<worktree-relative destination>]` so the worker links the project's gitignored environment file instead of hitting a structurally absent one; omit it for docs-only or read-only work.
