...
test_enrichment_preserves_all_unread_lines_and_status_file_failures() {
  ...
  assert_exact_line "$out" "$expected" "the oversized unread status line was truncated or omitted"
  i=1
  while [ "$i" -le 8 ]; do
    expected="wake annotation: latest wake-EVENT observed at drain, not current state: many-$i.status: $(cat "$state/many-$i.status")"
    assert_exact_line "$out" "$expected" "readable status many-$i was truncated or omitted"
    i=$((i + 1))
  done
