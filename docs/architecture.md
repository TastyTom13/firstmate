...
That declaration is authoritative for the stale path: it outranks both an active run-step attributed to the same crew and a still-live agent endpoint, for every kind of crew.
Those two readings are exactly what a real pause produces, which is why neither may override it: a crew pausing on its own pipeline's CI still reads `working` from that run, and a crew that pauses without exiting still has a live endpoint and may leave a background shell repainting the pane.
Overriding the declaration on either reading escalated healthy declared waits once per `FM_STALE_ESCALATE_SECS` and re-surfaced a bare stale wake on every new pane hash.
The declaration holds only while it is the crew's last status line, so a later `blocked:` or `needs-decision:` append surfaces at once and a `done:` followed by silence takes the ordinary stale path.
It is also bounded: at `FM_PAUSE_RESURFACE_SECS`, or at the `until` time a `paused:` line names when that comes first, the same pane re-surfaces once as a recheck naming which human the wait is on, so a forgotten pause or captain hold - including one on a secondmate, whose endpoint liveness is never read at all - cannot rot invisibly.
...
A trailing run of declared external waits is skipped only when the line under it is a `done` line, because a PR-raising ship worker is instructed to follow its done line with a merge wait, so that finished crew still reports `done` while a mid-task wait or a wait appended after any other verb still reports `paused`.
...
The away-mode exception for info-severity evidence-only findings is defined by the [afk classification policy](../.agents/skills/afk/SKILL.md#classification-policy), with shape recognition owned by `bin/fm-classify-lib.sh` and catch-up presentation by `bin/fm-afk-return.sh`.
...
`fm-send.sh` delivers every remote text steer and ordinary local text steer as a durable steering-inbox record plus a best-effort doorbell line (`bin/fm-task-inbox-lib.sh`).
The initial doorbell is constant, while watcher re-rings append the elapsed time since the steer so an unattended worker can assess age without inventing a time budget.
...
The [`fm-spawn.sh` header](../bin/fm-spawn.sh) owns ship/scout worktree isolation and fresh-base refusal rules, including spawns from linked homes and a fresh ship or scout started from an explicitly selected integration branch (`--base`) rather than the origin default branch.
...
Every delivery is attributed to the model that made it: both PR-raising modes' definition of done requires a `Built by: <harness>/<model> at <effort>` line in the PR body read from the task's own `state/<id>.meta`, and [`bin/fm-teardown.sh`](../bin/fm-teardown.sh) appends the same attribution as a `model=<model> effort=<effort>` note on every backlog Done close, so it outlives that record; [`bin/fm-model-scorecard.sh`](../bin/fm-model-scorecard.sh)'s header owns how those notes are tallied per model.
...
`/stow` sweeps the current session for durable knowledge that only exists in conversation, routes each finding to the most specific disk home, and prints one final copyable continuation prompt for the next session.
