...
It is also bounded: at `FM_PAUSE_RESURFACE_SECS` the same pane re-surfaces once as a recheck naming which human the wait is on, so a forgotten pause or captain hold - including one on a secondmate, whose endpoint liveness is never read at all - cannot rot invisibly.
...
Away-mode routing has one narrow exception to fail-safe escalation: a signal span whose actionable events are all info-severity, evidence-only ask-user findings is self-handled into the durable `state/.subsuper-self-handled` note that `bin/fm-afk-return.sh` presents on return, because `.agents/skills/ask-user-authority/SKILL.md` makes that shape firstmate's own call rather than the captain's; `bin/fm-classify-lib.sh` owns the shape test and fails closed on any other event, an undeclared severity, or an ask to change what is delivered.
...
`fm-spawn.sh` also owns the base-freshness boundary for every fresh ship and scout: no worker starts until its clean task worktree matches the fetched tip of the origin default branch or the explicitly selected integration branch, and any unsafe or unverifiable base stops the spawn.
