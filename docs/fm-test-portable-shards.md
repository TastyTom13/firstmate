...
The remainder has since grown to about 62 minutes of serial work, and on run [33494080823](https://github.com/TastyTom13/firstmate/actions/runs/33494080823) stale hints let `portable-serial-3of4` carry enough of it to hit that same 20-minute cap, so the split now uses six shards.
...
The 37 scripts that had no CI measurement at the 2026-09-16 upstream sync (the fork's own suites and upstream scripts newer than those runs) carry the durations measured by that sync's local serial-lane run on macOS; refresh them from the first green run of this fork's CI, since a CI runner is slower than that host.
...
for run in <run-id> <run-id> <run-id>; do
  gh run download "$run" -R TastyTom13/firstmate --pattern 'fm-test-timing-portable-serial-*' -D "/tmp/fm-serial/$run"
done
...
| portable serial 1-6 | job `timeout-minutes: 30` | Current runners can take about 20 minutes; the 30-minute cap remains a hang tripwire while leaving margin for job setup and runner-speed spread. |
