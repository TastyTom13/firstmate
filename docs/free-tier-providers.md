...
| Cerebras | Yes | No, by limited-purpose clause | 5 RPM / 1M tokens per day | Yes, OpenAI-compatible | **Recommended** |
| Scaleway Generative APIs | Yes | No, explicit | 1M tokens free | Yes, OpenAI-compatible | **Recommended**, EU/GDPR |
...
Free limits are 5 requests per minute and 1M tokens per day on `gpt-oss-120b`, plus $5 of credits that expire after 30 days.
...
The strongest written promise in the survey, and the only EU-jurisdiction one.
...
The free tier is *"up to 1,000,000 tokens for models billed by tokens"*; the surrounding examples are monthly bills, so treat the allowance as monthly and confirm on the console.
