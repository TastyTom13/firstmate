...
| Script                   | Purpose                                                                              |
| ------------------------ | ------------------------------------------------------------------------------------ |
| `fm-session-start.sh`    | Compose lock, bootstrap, wake drain, and the detect-only browser bridge summary into the single ordered session-start digest |
| `fm-chrome-bridge-sweep.sh` | Inventory and safely retire detached chrome-devtools-axi bridge families by recorded ownership and cwd |
...
| `fm-free-tier-guard.sh`  | Decide whether one brief may be dispatched to a free-tier provider, by repo allowlist and deny terms |
| `fm-free-lane-run.sh`    | Run one free-tier model lane through pi from a single command shape, and install the home-local `av bless` launcher that supplies its keys |
...
| `fm-context-budget.sh`   | Estimate how full the current Claude session's context is and throttle the turn-end /stow suggestion (docs/configuration.md) |
...
| `fm-pasted-content-lib.sh` | Single owner of the opt-in `<pasted_content>` marking that `fm-brief.sh` and `fm-send.sh` apply to text the captain pasted |
...
| `fm-model-scorecard.sh`  | Tally completed work per model and effort from the backlog's own attribution notes, with best-effort fix-round and ask-user proxies |
