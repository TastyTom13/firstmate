...
- Recovery of an existing presentation journal deliberately refuses the spawn when the shared presentation lock is still contended after a bounded wait sized to outlast one peer spawn, rather than falling back flat, and default-on makes that refusal reachable in any Herdr home.
...
Literal text and Enter are separate operations on `fm-send.sh`'s typed plane; ordinary local text steers instead use the durable steering inbox and send only its best-effort doorbell through this adapter, with elapsed time appended on watcher re-rings.
