...
# bounds that home's startup-memory curation, primary config/watch-thresholds
# gives that home the same supervision cadence, and primary
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context launch-env-allowlist claude-permission-mode watch-thresholds}"
